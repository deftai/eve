...
- `send(message, { auth, continuationToken, state? })` starts or resumes a session and returns its `Session`.
- `getSession(sessionId)` looks up an existing session. The returned `Session` exposes `getEventStream({ startIndex? })` for streaming.
- `cancelTurn({ sessionId })` requests cancellation of the session's currently running turn.
- `cancelSession({ sessionId })` requests cancellation of the entry session.
...
Cancellation is best effort. Requests return after the runtime accepts them, not after cleanup finishes, and can fail if they race hook registration or completion. A later reset operation that reuses the same stable continuation identity must wait for release before starting replacement work.
