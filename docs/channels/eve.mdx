...
An accepted cancellation returns `202`. This acknowledges the best-effort request but does not guarantee it takes effect — confirm the outcome from the stream (`turn.cancelled`, then `session.waiting` or `session.cancelled`). Turn cancellation while the session is parked is a no-op; an unavailable or completed session returns `409`.
