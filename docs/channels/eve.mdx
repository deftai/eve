...
- `POST /eve/v1/session/:sessionId/cancel` (cancel a turn or session)
...
Cancel the currently running turn with `{ "scope": "turn" }`, or cancel the complete entry session with `{ "scope": "session" }`. The route requires exactly one explicit scope.

An accepted cancellation returns `202`. This acknowledges the best-effort request but does not guarantee it takes effect. Cancelling active work emits `turn.cancelled`, followed by `session.waiting` or `session.cancelled`; cancelling an already parked session emits `session.cancelled` directly. Turn cancellation while the session is parked is a no-op. An unavailable or completed session returns `409`.
