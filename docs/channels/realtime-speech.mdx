...
`control: true` makes `/setup` mint a token carrying an Eve control URL + signed control token, and serves the `{basePath}/ws` route Gateway dials. Configuration:
...
- **`EVE_REALTIME_CONTROL_SECRET`** — HMAC secret for control tokens. Falls back to a derivation of `AI_GATEWAY_API_KEY` so previews work without extra config.
- **`VERCEL_AUTOMATION_BYPASS_SECRET`** (or `VERCEL_DPBP`) — when set, appended to the minted control URL as `x-vercel-protection-bypass` so Gateway can dial a protection-enabled preview. Temporary, for protected-preview testing.
