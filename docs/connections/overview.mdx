...
If a custom auth provider keeps its own token cache below eve's per-step cache, add `evict({ principal, connection })` to the auth definition. eve calls it after a rejected token, so the next `getToken` can refresh that cache. Leave it unset for static keys or providers without their own cache.
