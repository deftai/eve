...
- `t.start(input)` starts a turn without waiting and returns an active handle. Call `active.cancel()` to request cancellation of that turn and `active.result()` to observe its eventual boundary.
- `t.cancel()` requests cancellation of the complete entry session. A later send starts a fresh session after cancellation wins.
...
Cancellation is best effort and does not make `expectOk()` throw when it wins. A request that races hook registration or completion can return `409`:

```ts
const active = await t.start("Keep working until cancelled.");
await active.cancel();
const turn = await active.result();

if (turn.status !== "cancelled") throw new Error("Expected turn cancellation.");
```
