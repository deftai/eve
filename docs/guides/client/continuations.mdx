...
`ClientSession` also tracks `streamIndex`, the count of physical events already consumed, and a
bounded logical event cursor used to hide Workflow replays. Together these fields make a
`SessionState`.
...
interface SessionState {
  ...
  eventCursor?: SessionEventCursor;
  sessionId?: string;
  streamIndex: number;
}
```

The continuation token resumes the conversation. The session ID and stream index let the client
reconnect to the right physical stream position. `eventCursor` retains only the active turn's event
IDs and a settled-turn watermark, so its size does not grow with the conversation. Treat it as opaque
client state and store the full object rather than rebuilding selected fields.
