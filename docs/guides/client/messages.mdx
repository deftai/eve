...
| `status`    | `"waiting"`, `"completed"`, `"failed"`, or `"cancelled"`.                      |
...
Cancel only the turn represented by a response with `MessageResponse.cancel()`. When cancellation wins, the session remains resumable and the result settles with `status: "cancelled"` after `turn.cancelled` and `session.waiting`:
...
Cancel the complete entry session with `ClientSession.cancel()`. This clears the client's continuation cursor; the active result ends at `session.cancelled`, and the next `send()` starts a new session:

```ts
await session.cancel();
```

Both methods make a best-effort durable server-side cancellation request. A request that races hook registration or completion can return `409`. An `AbortSignal` passed to `send()` only stops the local HTTP request or stream and does not cancel server-side work.
