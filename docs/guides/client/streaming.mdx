...
The durable stream is an at-least-once physical log: Workflow may append concurrent or late copies
of events from the same logical turn. The client consumes those copies, advances its physical cursor,
and suppresses them before they reach iteration, `result()`, evals, or frontend stores. Equal payloads
from distinct logical events are preserved; filtering uses server-provided event identity and turn
coordinates, not payload hashing.

Persist the complete `session.state` after consuming a response. In addition to `streamIndex`, it
contains a bounded logical cursor that keeps replay filtering correct after a process restart. A
saved state from before this cursor existed is upgraded by silently reconstructing it from the
durable stream prefix once.
...
`startIndex` is a physical offset. Overriding it to a different position also reconstructs the
logical cursor through that prefix before exposing events.
