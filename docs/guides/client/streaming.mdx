...
| `turn.cancelled`     | Mark the active turn intentionally cancelled.                    |
| `session.waiting`    | Enable the composer for the next turn.                           |
| `session.completed`  | Mark the conversation terminal.                                  |
| `session.cancelled`  | Mark the complete entry session intentionally cancelled.         |
...
This does not cancel the model, tools, or workflow on the server. Use `response.cancel()` for the active turn or `session.cancel()` for the complete entry session when server-side work must stop.
