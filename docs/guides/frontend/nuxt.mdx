...
| --------------------- | --------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `eveRoot`             | `string`  | Nuxt project root      | Path to the eve app root, relative to the Nuxt project root unless absolute. Set it when the agent lives outside the Nuxt project.                                              |
...
| `configureVercelJson` | `boolean` | `true`                 | Creates or updates `vercel.json` with experimental services for the Nuxt app and eve app. Set it to `false` when you manage `vercel.json` yourself.                            |
| `servicePrefix`       | `string`  | `"/_eve_internal/eve"` | Private Vercel route namespace for the eve service. When `configureVercelJson` runs, the module writes it as `routePrefix` unless `vercel.json` already has one. It cannot be `/`. |
