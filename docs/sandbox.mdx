...
This managed form belongs specifically to `vercel()`; a generic sandbox backend is not required to implement it. Do not also set `networkPolicy` through `onSession` or `setNetworkPolicy` when using managed brokering. Vercel network policy updates replace the entire policy, so doing this removes the brokered transforms and disables Vercel Connect credential injection for sandbox egress.
