...
With the Vercel backend, attach authorization directly to each route that needs it:
...
`auth` uses the same authorization contract as connections and authored tools: a static `getToken`, a per-user strategy, or an interactive strategy such as Vercel Connect. Eve resolves every authenticated rule before returning the sandbox to authored code. Interactive credentials park and resume through Eve's normal callback lifecycle. Native Vercel `match` filters remain available on both authenticated and unauthenticated rules.

Firewall transformations require Vercel Pro or Enterprise and are unavailable on Hobby.

Eve removes resolved tokens after every step. Routes whose credentials are unavailable remain closed. Managed policies reject replacement through `onSession({ use })` or `sandbox.setNetworkPolicy()`; a second policy writer would otherwise remove Eve's transforms. Brokered credentials are unavailable during template bootstrap.
