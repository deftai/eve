...
For frequent checks that usually have nothing to report, opt the turn into empty delivery. eve exposes `skip_delivery` only for that turn; calling it completes successfully without posting a channel message, while emitting `delivery.skipped` on the session stream.

```ts
receive(slack, {
  message: "Check for new incidents. Report them, or call `skip_delivery` if there are none.",
  target: { channelId: "C0123ABC" },
  auth: appAuth,
