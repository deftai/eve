...
This is the minimal schedule. eve runs the agent on the prompt and throws away the output, though the agent can still call tools, write to backends, and log along the way. We call this task mode. A task-mode session runs to completion or fails unless it parks for a framework callback such as connection authorization. It cannot ask the channel for human input, so human-in-the-loop (HITL) approvals and `ask_question` prompts fail instead of waiting.
...
A handler-form session runs on the same durable runtime engine as any other session, so it can park (durably suspend), for instance when the channel handoff is waiting for a Slack reply. Markdown task mode still cannot wait for channel-delivered human input.
