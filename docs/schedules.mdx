...
For frequent checks that usually have nothing to report, opt the turn into empty delivery. eve buffers the final response and posts it only when the agent decides there is something to report. A skipped delivery completes successfully and emits `delivery.skipped` on the session stream.

```ts
receive(slack, {
  message: "Check for new incidents. Report them, or send nothing if there are none.",
  target: { channelId: "C0123ABC" },
  auth: appAuth,
