...
Each delegated subagent spins up its own child session and stream. The parent stream carries `subagent.called`, `action.result`, and `subagent.completed` for the delegation. If the child asks for human input, eve proxies an `input.requested` event onto the parent stream and routes the matching response back to the child. To follow the child's full progress, read `subagent.called.data.childSessionId` and subscribe at `GET /eve/v1/session/:childSessionId/stream`.
