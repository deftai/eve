...
import { pickProject, pickTeam, resolveProjectByNameOrId } from "#setup/vercel-project.js";
...
async function selectProject(
...
): Promise<{ readonly project: string; readonly team: string }> {
  ...
  return { project: picked.project, team };
}
  ...
  try {
    ...
    if (shouldSelectProject) {
      try {
        ...
        projectAuthority = { orgId: resolved.orgId, projectId: resolved.projectId };
      } catch (error) {
    ...
    if (token.length === 0) {
      return failed(
        "Vercel did not issue an OIDC token for the selected project.",
        completedMutations,
      );
    }
    const resolveToken = async (): Promise<string> => {
      const refreshed = (await deps.resolveOidcToken(target.deployment)).trim();
      if (refreshed.length > 0) token = refreshed;
      return token;
    };
