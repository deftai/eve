...
/**
 * Global symbol the generated module map sets while evaluating an extension's
 * modules, so `defineState` can namespace durable keys by the owning extension.
 */
const EXT_STATE_SCOPE_GLOBAL = 'Symbol.for("eve.ext-state-scope")';

/**
 * Extracts the extension namespace from a composed source id (`ext:<ns>:…`),
 * or `null` for consumer-owned modules.
 */
function extensionScopeForSourceId(sourceId: string): string | null {
...
interface CollectedModuleImport {
  ...
  /** Extension namespace to scope this module's evaluation under, if any. */
  readonly extScope: string | null;
}
...
export function createCompiledModuleMapSource(input: CreateCompiledModuleMapSourceInput): string {
  ...
  const staticModules = allModules.filter((moduleImport) => moduleImport.extScope === null);
  const scopedModules = allModules.filter((moduleImport) => moduleImport.extScope !== null);

  const staticImports = staticModules.map(
    (moduleImport) =>
      `import * as ${moduleImport.bindingName} from ${JSON.stringify(moduleImport.importSpecifier)};`,
  );

  // Extension-owned modules are imported dynamically inside a namespace scope so
  // `defineState` can prefix durable keys by extension. Consumer-only agents
  // have no scoped modules, so this section is omitted and the output is
  // byte-identical to a non-extension build.
  const scopedImports =
    scopedModules.length === 0
      ? []
      : [
          `const __eveExtStateScope = ${EXT_STATE_SCOPE_GLOBAL};`,
          ...scopedModules.flatMap((moduleImport) => [
            `globalThis[__eveExtStateScope] = ${JSON.stringify(moduleImport.extScope)};`,
            `const ${moduleImport.bindingName} = await import(${JSON.stringify(moduleImport.importSpecifier)});`,
            `globalThis[__eveExtStateScope] = undefined;`,
          ]),
        ];
