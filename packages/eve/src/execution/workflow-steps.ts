 ...
 * conversation deliveries — including HITL response turns — replace the
 * previous turn's policy. Runtime-owned continuation steps and task-mode HITL
 * responses preserve whatever is already in effect because their callers
 * cannot attach the original run-scoped schema again.
 */
export function resolveEffectiveOutputSchema(input: {
  ...
  if (
    ...
    (mode === "task" && stepInput.inputResponses !== undefined)
  ) {
    return session;
  }
