...
- `task spec:render` — Regenerate `SPECIFICATION.md` from `specification.xbrief.json`
- `task roadmap:render` — Regenerate `ROADMAP.md` from `xbrief/pending/` scope vBRIEFs
- `task project:render` — Refresh `PROJECT-DEFINITION.xbrief.json` items registry from lifecycle folders
- `task migrate:vbrief` — Migrate existing project to vBRIEF lifecycle folder structure (one-time)
- `task issue:ingest -- <N>` / `task issue:ingest -- --all [--label L] [--status S] [--dry-run]` — Ingest GitHub issues as scope vBRIEFs in `xbrief/proposed/` (deduplicates via existing references)
...
- `task scope:decompose -- <parent.xbrief.json> --draft xbrief/.eval/decompositions/<parent-slug>.json` — Apply an approved phase/epic to story decomposition
- `task swarm:readiness -- xbrief/active/*.xbrief.json` — Report whether candidate stories are safe for concurrent swarm allocation
...
All vBRIEF files live in `./xbrief/` within the project workspace. Files are organized into **singular operational files** at the vbrief root and **scope vBRIEFs** in lifecycle folders.

### Directory Structure

```
xbrief/
  PROJECT-DEFINITION.xbrief.json   <- project identity gestalt
  specification.xbrief.json        <- project spec source of truth
  specification-{name}.xbrief.json <- add-on specs
  plan.xbrief.json                 <- session-level tactical plan (singular)
  continue.xbrief.json             <- interruption checkpoint (singular, ephemeral)
  playbook-{name}.xbrief.json      <- reusable operational patterns
...
| ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| `PROJECT-DEFINITION.xbrief.json`   | Project identity gestalt — `narratives` for identity (overview, tech stack, architecture, risks/unknowns, config), `items` as scope registry; uses the canonical v0.6 schema | Durable (regenerated on demand) |
| `specification.xbrief.json`        | Project spec source of truth                                                                                                                                                 | Durable (never deleted)         |
| `specification-{name}.xbrief.json` | Add-on spec with `planRef` back to main spec                                                                                                                                 | Durable                         |
| `plan.xbrief.json`                 | Session-level tactical plan; the _how right now_; carries `planRef` to scope vBRIEFs                                                                                         | Session-durable                 |
| `continue.xbrief.json`             | Interruption recovery checkpoint; carries `planRef` to scope vBRIEFs                                                                                                         | Ephemeral (consumed on resume)  |
| `playbook-{name}.xbrief.json`      | Reusable operational knowledge                                                                                                                                               | Permanent                       |
...
- ! When moving a file, agents MUST update all `planRef` and `references[].uri` values in other scope vBRIEFs and in `PROJECT-DEFINITION.xbrief.json` that point to the moved file
...
- ! Scope vBRIEF filenames MUST follow: `YYYY-MM-DD-descriptive-slug.xbrief.json`
- ! The date MUST be the **creation date** (immutable — does not change as the scope progresses)
- ~ Use lowercase hyphen-separated slugs (e.g. `2026-04-12-add-oauth-flow.xbrief.json`)
...
- ! Filename MUST follow `YYYY-MM-DD-ip<NNN>-<slug>.xbrief.json` where `<NNN>` is the implementation phase index zero-padded to **exactly 3 digits** (e.g. `ip001`, `ip007`, `ip042`, `ip128`).
...
- ~ The `<slug>` SHOULD describe the IP, not the issue (e.g. `ip003-data-layer.xbrief.json`, not `ip003-issue-123.xbrief.json`).
- ~ Use lowercase hyphen-separated slugs (e.g. `2026-04-12-ip003-data-layer.xbrief.json`).
...
Canonical reference types (all prefixed `x-xbrief/` per the v0.6 schema): `x-xbrief/github-issue`, `x-xbrief/github-pr`, `x-xbrief/jira-ticket`, `x-xbrief/user-request`, `x-xbrief/plan`, `x-xbrief/spec-section`. See [`../conventions/references.md`](../conventions/references.md) for the full type registry.

**Platform note:** The migration script (`task migrate:vbrief`) defaults origin provenance to `x-xbrief/github-issue` type. Non-GitHub users should manually adjust `references[].type` in generated vBRIEFs after migration. See [README.md — Platform Requirements](../../README.md#%EF%B8%8F-platform-requirements).
...
- `external` -- originated from outside the trust boundary: GitHub issue / PR body or comment, web page, third-party documentation, retrieved file, tool output that quoted external content, sibling-agent message quoting external content. The default for any `references[].type == "x-xbrief/github-issue"` ingest path; the default for any agent-side write that incorporates content the agent did not author
...
"references": [
  ...
  {
    "uri": "completed/2026-05-12-481-patterns-directory-and-llm-app-standards.xbrief.json",
    "type": "x-xbrief/plan",
...
- ! Every reference produced by an external-source ingest path (e.g. `task issue:ingest`, web-research import, third-party-doc paste) MUST carry `"TrustLevel": "external"` at write time -- the default for any `x-xbrief/github-issue` or `x-xbrief/web-page` reference
- ! References to other in-repo vBRIEFs (`x-xbrief/plan`, `x-xbrief/spec-section`) MUST carry `"TrustLevel": "internal"` -- they originated inside the trust boundary
...
- ! All `uri` and `planRef` path values in scope vBRIEF JSON are **relative to the `xbrief/` directory** — not relative to the containing file's location
- ! Epic `references` array MUST list child story file paths (type: `x-xbrief/plan`, `TrustLevel: internal`)
...
- ! traces MUST exist through item/story `Traces`, `x-xbrief/spec-section` references, or an explicit `missing_traces_justification`
- ! `planRef` MUST point to the parent phase/epic when the story was decomposed from one
- ! parent phase/epic `references` MUST include child story paths with `type: x-xbrief/plan` and `TrustLevel: internal`
...
- ⊗ Use placeholder acceptance such as "to refine from parent scope", criteria identical to the title/description, vague docs-only acceptance, broad file globs such as `backend/**`, `frontend/**`, `docs/**`, `xbrief/**`, or generic verification such as only `task check`.
...
```json
{
  ...
  "plan": {
    ...
    "references": [
      {
        "type": "x-xbrief/plan",
        "uri": "active/2026-04-12-oauth-flow.xbrief.json",
        "TrustLevel": "internal"
      },
      {
        "type": "x-xbrief/plan",
        "uri": "active/2026-04-12-session-mgmt.xbrief.json",
        "TrustLevel": "internal"
      }
    ]
  }
...
```json
{
  ...
  "plan": {
    ...
    "status": "running",
    "planRef": "active/2026-04-10-auth-system-overhaul.xbrief.json"
  }
}
```

### Coexistence: Scope vBRIEFs, plan.xbrief.json, and continue.xbrief.json

Scope vBRIEFs are durable scope records (the _what_); `plan.xbrief.json` remains the ephemeral session-level tactical plan (the _how right now_); `continue.xbrief.json` remains the interruption checkpoint. Both gain a parent reference to scope vBRIEFs via `planRef`.

- **Scope vBRIEF** — acceptance criteria, scope definition, origin provenance. Durable across sessions. Shared between agents.
- **plan.xbrief.json** — granular implementation steps for this session. Session-durable. Agent-private.
- **continue.xbrief.json** — interruption checkpoint. References scope vBRIEF(s) being worked on.

- ! When scope vBRIEFs exist, plan.xbrief.json and continue.xbrief.json MUST carry a `planRef` to the scope vBRIEF(s) they relate to
- ⊗ Use scope vBRIEFs as session scratchpads — that is what plan.xbrief.json is for
...
3. Agent drafts a temporary decomposition proposal under `xbrief/.eval/decompositions/<parent-slug>.json`
4. Agent presents the draft to the user and gets explicit approval
5. `task scope:decompose -- <parent> --draft xbrief/.eval/decompositions/<parent-slug>.json --check` validates the approved draft
6. `task scope:decompose -- <parent> --draft xbrief/.eval/decompositions/<parent-slug>.json` creates child story vBRIEFs with `planRef` back to parent
7. Parent epic's `references` updated to list all child paths
8. Update `plan.xbrief.json` (and `continue.xbrief.json` if present) `planRef` to reference child scope vBRIEFs
...
- ! Agents SHOULD write decomposition draft proposals under `xbrief/.eval/decompositions/`
- ! Derive `<parent-slug>` from the parent vBRIEF filename by removing `.xbrief.json` and any leading `YYYY-MM-DD-` date prefix; for example, `2026-05-12-ip001-auth.xbrief.json` becomes `ip001-auth`
- ⊗ Agents MUST NOT leave decomposition draft JSON files at the workspace root
- ! Generated child story vBRIEFs remain lifecycle artifacts and default to `xbrief/pending/`
...
- ! All vBRIEF files MUST live in `./xbrief/` or its lifecycle subfolders — never in workspace root
- ! File names MUST use the `.xbrief.json` extension
- ⊗ Use ULID or timestamp suffixes on `continue` or `plan` — they are singular by design
- ⊗ Create multiple `plan.xbrief.json` files — there is exactly one active plan
- ⊗ Create a separate `todo-*.json` — todos live in `plan.xbrief.json`

---

## File Format

All `.xbrief.json` files conform to the **vBRIEF v0.6** specification.
...
- ⊗ Edit `SPECIFICATION.md` directly — edit the source `specification.xbrief.json` instead
- ? Create `specification-{name}.xbrief.json` for add-on specs (e.g. security, deployment)
    ...
    "items": [
      {
        ...
        "status": "running",
        "references": [
          { "type": "x-xbrief/plan", "uri": "active/2026-04-12-add-oauth-flow.xbrief.json" }
        ]
      }
    ]
  }
...
- ! Singular — exactly one per project at `./xbrief/` root
...
The single active work plan. Unifies what were previously separate todo, plan, and progress files. When scope vBRIEFs are in use, plan.xbrief.json is the session-level tactical plan (the _how right now_) and carries a `planRef` to the scope vBRIEF(s) being implemented.

**Status lifecycle per task:** `pending` → `running` → `completed` / `blocked` / `failed` / `cancelled`

- ! There is exactly ONE `plan.xbrief.json` at a time per project
...
## continue.xbrief.json

A single interruption-recovery checkpoint. See [resilience/continue-here.md](../resilience/continue-here.md)
for full protocol. When scope vBRIEFs are in use, continue.xbrief.json carries a `planRef` to the scope vBRIEF(s) the agent was working on.

- ! Singular — `continue.xbrief.json`, not `continue-{ULID}.json`
...
## playbook-{name}.xbrief.json

Reusable operational patterns. Examples: `playbook-deploy.xbrief.json`, `playbook-release.xbrief.json`.
...
Interview (strategies/interview.md, Light path)
        │
        ▼
./xbrief/specification.xbrief.json   ← status: draft
        │
   user reviews
        │
        ▼
./xbrief/specification.xbrief.json   ← status: approved
        │
   task spec:render
        │
        ▼
...
```
./xbrief/specification-{name}.xbrief.json  →  SPECIFICATION-{name}.md
```
...
| ----------------------- | ---------------------------------------------- |
| `create_todo_list`      | write `./xbrief/plan.xbrief.json`              |
| `mark_todo_as_done`     | update task `status` → `completed`             |
| `add_todos`             | append task to `./xbrief/plan.xbrief.json`     |
| `remove_todos`          | set task `status` → `cancelled` (never delete) |
| session end / interrupt | write `./xbrief/continue.xbrief.json`          |
| spec interview output   | write `./xbrief/specification.xbrief.json`     |

---

## Anti-Patterns

- ⊗ Placing vBRIEF files in workspace root (`./plan.xbrief.json`, `./progress.xbrief.json`)
- ⊗ Using ULID suffixes on `plan`, `continue`, or `todo` files — they are singular
- ⊗ Creating `todo-{ULID}.json` — todos live in `plan.xbrief.json`
- ⊗ Editing `SPECIFICATION.md` directly — it is a generated artifact
- ⊗ Treating `plan.xbrief.json` as a scratch file and deleting it mid-task
- ⊗ Creating both a `plan.xbrief.json` and a separate `progress.xbrief.json` — they are the same file
- ⊗ Moving scope vBRIEFs between lifecycle folders without updating `plan.status`
- ⊗ Using scope vBRIEFs as session scratchpads — use plan.xbrief.json for tactical session work
- ⊗ Creating scope vBRIEFs without origin provenance (`references` linking to the origin)
- ⊗ Omitting `planRef` from plan.xbrief.json or continue.xbrief.json when scope vBRIEFs exist
